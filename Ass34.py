...
def main():
    ...
    sys.stdout=open("testfile_naivebayes.txt","w")
    #ident=(np.identity(covar_virginica.shape[0]))
    covar_virginica=np.diag(np.diag(covar_virginica))
    covar_setosa=np.diag(np.diag(covar_setosa))
    covar_versicolor=np.diag(np.diag(covar_versicolor))
